...
// debug
#ifndef ATTN_B
constexpr int ATTN_B__1 = 1;//16; // batch size
#endif
#ifndef ATTN_H
constexpr int ATTN_H__1 = 1;  // number of heads
#endif
#ifndef ATTN_D
...
constexpr int V_CHUNK_SIZE__32 = 32;
...
__forceinline__ __device__ void print_mem(const __hip_bfloat16 *ptr){
    if(threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0) { 
        printf("shared_data_ptr: %p\n", ptr);
        for(int i = 0; i < row; i ++) {
            if(i % 8 == 0 && i != 0) {
                printf("\n");
            }
...
#define DUMP_KV_STATE_SUM(MSG) {                                      \
    if (threadIdx.x == 0 && threadIdx.y == 0) {                       \
        ...
        for (int i = 0; i < ATTN_F__128 * V_CHUNK_SIZE__32; i++) {             \
...
template<int D, typename T=bf16, typename L=row_l, typename S=rt_32x16_s> using v_tile = rt<T, CHUNK_SIZE__64, V_CHUNK_SIZE__32, L, S>;
...
template<int D, typename T=bf16, typename L=row_l, typename S=rt_32x16_s> using o_tile = rt<T, CHUNK_SIZE__64, V_CHUNK_SIZE__32, L, S>;
template<int D, typename T=bf16, typename L=col_l, typename S=rt_16x32_s> using o_tile_transposed = rt<T, V_CHUNK_SIZE__32, CHUNK_SIZE__64, L, S>;
template<int D, typename T=float, typename L=col_l, typename S=rt_16x32_s> using kv_state_tile = rt<T, ATTN_F__128, V_CHUNK_SIZE__32, L, S>;
...
//     gl<bf16, -1, -1, -1, -1> Qg, Kg, K_split_g, Vg, Og;
//     gl<bf16, -1, -1, -1, -1> ODEBUGg;
...
//     dim3 grid() {return dim3(ATTN_D__128 / V_CHUNK_SIZE__32, ATTN_H__1, ATTN_B__1);} // parallel in V's ATTN_D dim
...
using GL_Q = gl<bf16, 1, 64, 1, 128>;
using GL_K = gl<bf16, 1,  64, 1, 128>;
using GL_V = gl<bf16, 1,  64, 1, 128>;
using GL_OUT = gl<bf16, 1, 64, 1, 128>;

__global__ 
__launch_bounds__(NUM_THREADS, 2)
void lightning_attn2_kernel(
    bf16* __restrict__ q_ptr, 
    bf16* __restrict__ k_ptr,
    bf16* __restrict__ v_ptr,
    // bf16* __restrict__ s_ptr,
...
)
{
    ...
    GL_K Kg = GL_K(k_ptr);
    GL_V Vg = GL_V(v_ptr);
    GL_OUT Og = GL_OUT(out_ptr);
    extern __shared__ alignment_dummy __shm[];
    shared_allocator al((int*)&__shm[0]);

    // smem
    st_bf<CHUNK_SIZE__64, ATTN_F__128, st_32x32_s> (&q_smem)[2] = al.allocate<st_bf<CHUNK_SIZE__64, ATTN_F__128, st_32x32_s>, 2>();                   // 64x128 x2 x2 = 32768 B = 32 KB
    st_bf<CHUNK_SIZE__64, ATTN_F__128, st_32x32_s> (&k_smem)[2] = al.allocate<st_bf<CHUNK_SIZE__64, ATTN_F__128, st_32x32_s>, 2>();                   // 64x128 x2 x2 = 32768 B = 32 KB
    st_bf<CHUNK_SIZE__64, V_CHUNK_SIZE__32, st_32x32_s> (&v_smem)[2] = al.allocate<st_bf<CHUNK_SIZE__64, V_CHUNK_SIZE__32, st_32x32_s>, 2>();       // 64x32 x2 x2 = 8192 B = 8 KB
    st_bf<ATTN_F__128, V_CHUNK_SIZE__32, st_32x32_s> (&kv_state_smem) = al.allocate<st_bf<ATTN_F__128, V_CHUNK_SIZE__32, st_32x32_s>>();              // 128x32 x2 = 8192B = 8KB
    ...
    row_vec<st_fl<ATTN_D__128, CHUNK_SIZE__64, st_32x32_s>> (&q_decay) = 
                al.allocate<row_vec<st_fl<ATTN_D__128, CHUNK_SIZE__64, st_32x32_s>>>();
    row_vec<st_fl<ATTN_D__128, CHUNK_SIZE__64, st_32x32_s>> (&k_decay) = 
                al.allocate<row_vec<st_fl<ATTN_D__128, CHUNK_SIZE__64, st_32x32_s>>>();
    constexpr int sizeof_shared__142000 = sizeof(q_smem) + sizeof(k_smem) + sizeof(v_smem) + sizeof(kv_state_smem) + sizeof(q_decay) + sizeof(k_decay);// + sizeof(kv_state_smem222);
    static_assert(sizeof_shared__142000 < 160000);
    // decay in register
    ...
    float slope = 0; //reinterpret_cast<float*>(s_ptr)[head_idx];

    constexpr int N = 64;
    int blocks = N / CHUNK_SIZE__64;
    const int tic__0 = 0, toc = 1;

    // Initialize all of the register tiles.
    q_tile<ATTN_F__128, bf16> q_reg;                         // [CHUNK_SIZE, ATTN_F], 64x128,                            64x128/64/2 = 64 VGPRs
    q_tile_transposed<ATTN_F__128, bf16> q_reg_transposed;   // [ATTN_F, CHUNK_SIZE], 128x64, rt_16x32_s, 8x2 subtiles,  64 VGPRs
    k_tile<ATTN_F__128, bf16> k_reg;                         // [CHUNK_SIZE, ATTN_F], 64x128,                            64x128/64/2 = 64 VGPRs
    k_tile_transposed<ATTN_F__128, bf16> k_reg_transposed;   // [ATTN_F, CHUNK_SIZE], 128x64,                            64 VGPRs
    
    v_tile<ATTN_D__128, bf16, col_l, rt_32x32_s> v_reg;                      // [CHUNK_SIZE, V_CHUNK_SIZE], 64x32,       64x32/64/2 = 16 VGPRs
    o_tile_transposed<ATTN_D__128, float, col_l, rt_32x32_s> o_intra;        // [V_CHUNK_SIZE, CHUNK_SIZE], 32x64,       32x64/64 = 32 VGPRs
    o_tile_transposed<ATTN_D__128, float, col_l, rt_32x32_s> o_inter;        // [V_CHUNK_SIZE, CHUNK_SIZE], 32x64,       32x64/64 = 32 VGPRs
    attn_tile<ATTN_D__128, float, col_l, rt_32x32_s> attn_block[1];          // [CHUNK_SIZE, CHUNK_SIZE], 64x64,         64x64/64 = 64 VGPRs
    attn_tile<ATTN_D__128, bf16, col_l, rt_32x32_s> attn_block_bf16;         // [CHUNK_SIZE, CHUNK_SIZE], 64x64,         64x64/64/2 = 32 VGPRs

    // rt_bf<ATTN_F, V_CHUNK_SIZE, col_l, rt_16x32_s> local_kv_reg; // [ATTN_F, V_CHUNK_SIZE], 128x32, 8x1 subtiles
    rt_bf<ATTN_F__128/4, V_CHUNK_SIZE__32, col_l, rt_16x32_s> local_kv_reg; // [ATTN_F/4, V_CHUNK_SIZE], 32x32, 2x1 subtiles,32x32/64/2 = 8 VGPRs
    ...
    using T = typename st_bf<CHUNK_SIZE__64, ATTN_F__128, st_32x32_s>::dtype;
    constexpr int bytes_per_thread = st_32x32_s::template bytes_per_thread<T>();
    constexpr int bytes_per_memcpy = bytes_per_thread * NUM_THREADS;
    constexpr int memcpy_per_tile_q_k = CHUNK_SIZE__64 * ATTN_F__128 * sizeof(T) / bytes_per_memcpy;
    // constexpr int memcpy_per_tile_v = CHUNK_SIZE * ATTN_D * sizeof(T) / bytes_per_memcpy;
    constexpr int memcpy_per_tile_v = CHUNK_SIZE__64 * V_CHUNK_SIZE__32 * sizeof(T) / bytes_per_memcpy;
    ...
    // G::prefill_swizzled_offsets<1, false>(q_smem[0], Qg, swizzled_offsets_Q);
    ...
    if (warpid == 1) {
        ...
        add(k_decay_rv, k_decay_rv, CHUNK_SIZE__64);    // 64,63,62,...,1
...
// #endif
    ...
    // if(threadIdx.x == 0) printf("Vg addr: %p", v_ptr);
    for (int block = 0; block < blocks; block++) {
        ...
        G::load<1, false>(q_smem[tic__0], Qg, {batch_idx, block, head_idx, 0});
        G::load<1, false>(k_smem[tic__0], Kg, {batch_idx, block, head_idx, 0});
        // G::load<1, false>(v_smem[tic], Vg, {batch_idx, block, head_idx, v_block_idx});
        G::load<1, false>(v_smem[tic__0], Vg, {batch_idx, block, head_idx, 0});
        BARRIER;
#ifdef DEBUG
        if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        ...
        // smem to reg
        load(q_reg, q_smem[tic__0]);
        load(k_reg, k_smem[tic__0]);
        load(v_reg, v_smem[tic__0]);
        BARRIER;
#ifdef DEBUG
        if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        ...
        BARRIER;
        // print_mem<64, 32, 128>(q_smem[0].data);
        // calculation QK
        zero(attn_block[tic__0]);
...
// #endif
        ...
        //             D(attn_block[0].tiles[j][0].data[i].x);
        //             D(attn_block[0].tiles[j][0].data[i].y);
        //         }
...
//         // mul_col(o_inter, o_inter, q_decay_rv); // currently commented out for o_inter debug dump
        ...
        // mma_AtB(o_inter, local_kv_reg, q_reg_transposed, o_inter); // q_reg_transposed [ATTN_F, CHUNK_SIZE], 128x64, o_inter is (KV)^T*Q^T
        mma_AtB(o_inter, local_kv_reg, subtile_inplace<ATTN_F__128/4>(q_reg_transposed, 0), o_inter); // local_kv_reg [ATTN_F/4, V_CHUNK_SIZE], q_reg_transposed [ATTN_F, CHUNK_SIZE], 128x64, o_inter is (KV)^T*Q^T

        load(local_kv_reg, subtile_inplace<ATTN_F__128/4, V_CHUNK_SIZE__32>(kv_state_smem, {1, 0}));
        BARRIER;
        mma_AtB(o_inter, local_kv_reg, subtile_inplace<ATTN_F__128/4>(q_reg_transposed, 1), o_inter);

        load(local_kv_reg, subtile_inplace<ATTN_F__128/4, V_CHUNK_SIZE__32>(kv_state_smem, {2, 0}));
        BARRIER;
        mma_AtB(o_inter, local_kv_reg, subtile_inplace<ATTN_F__128/4>(q_reg_transposed, 2), o_inter);

        load(local_kv_reg, subtile_inplace<ATTN_F__128/4, V_CHUNK_SIZE__32>(kv_state_smem, {3, 0}));
        BARRIER;
        ...
        rt_fl<ATTN_F__128, V_CHUNK_SIZE__32, col_l, rt_32x32_s> local_kv;        // 128x32
        ...
        // float block_decay = __expf(-slope * static_cast<float>(CHUNK_SIZE__64));
        BARRIER
        // print_mem<128, 32, 32>(kv_state_smem.data);
        BARRIER
       ...
       if (threadIdx.x == 0 && threadIdx.y == 0) {
        ...
        // o_reg_transposed.tiles[0][0].data[0].x += kv_state_sum;
    }
...
// #endif
        ...
        // store<1>(ODEBUGg, o_inter_transposed, {batch_idx, block, head_idx, 0});
        BARRIER
        // print_mem<128, 32, 32>(kv_state_smem.data);
//     // print_mem<
// #ifdef DEBUG
...
// //         // 1. Read slopes
// //         for(int i = 0; i < ATTN_H__1; i++) {
// //             infile >> slopes[i];
// //             printf("slopes[%d] = %f\n", i, slopes[i]);
// //         }
// //         std::cout << "Finished loading " << ATTN_H__1 << " slopes" << std::endl;
